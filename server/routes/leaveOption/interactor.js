...
const createLeaveOption = async ( req, res )=>{
  try {
    ...
    if( validation.valid ) {
      ...
      const employeeIdList = await db.find( `select employee_id from employee_basic_details;` )
...
const deleteLeaveOption = async ( req, res )=>{
  try {
    ...
    if( validation.valid ) {
      const optionDetail = ( await db.find( `select * from leave_options where option_id=${ data.option_id };` ) )[0]
      await db.run( `delete from leave_options where option_id=${ data.option_id };` )
      await db.run( `delete from available_leave where type='${ optionDetail.name }'` )
